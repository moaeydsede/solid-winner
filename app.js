...
export const money = (val, ccy)=>{
  try{ return new Intl.NumberFormat("ar-EG", {style:"currency", currency:ccy, maximumFractionDigits:0}).format(n(val)); }
  catch{ return (n(val)).toFixed(0)+" "+ccy; }
};
...
export function rangeForPeriod(kind, anchorDateStr){
  ...
  if(kind==="month"){ startM=m; endM=m; }
  if(kind==="quarter"){ const q = Math.floor(m/3); startM=q*3; endM=q*3+2; }
  if(kind==="half"){ const h = m<6 ? 0 : 1; startM=h*6; endM=h*6+5; }
  if(kind==="year"){ startM=0; endM=11; }
  const start = `${y}-${pad(startM+1)}-01`;
  const end = new Date(y, endM+1, 0).toISOString().slice(0,10);
  return {start, end};
}
